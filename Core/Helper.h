...
// 예외 보고에 사용되는 여러 표준 클래스를 정의한 라이브러리
#include <stdexcept>

// ComPtr은 CPU에서 자원의 수명을 관리하는 데 사용됩니다.
// 하지만 GPU에서 사용되는 자원의 수명에 대해 ComPtr은 이해하지 못합니다.
// 앱에서는 GPU에서 여전히 참조될 수 있는 객체를 파괴하지 않도록 주의해야
// 합니다. 즉, GPU에서 사용 중인 자원의 수명을 따로 관리해야 합니다.
...
// ComPtr<T>의 이름 지정 도우미입니다.
// 변수의 이름을 객체의 이름으로 할당합니다.
// 인덱스화된 변수는 객체의 이름에 인덱스를 포함합니다.
...
// 모든 Comptr 배열 요소의 Reset()를 호출합니다.
template<class T>
...
// 모든 uniaue_ptr 배열 요소의 Reset()를 호출합니다.
template<class T>
