...
std::wstring Utility::UTF8ToWideString(const std::string& str)
{
    ...
    if (!MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, str.c_str(), -1, wstr, MAX_PATH)) wstr[0] = L'\0';
    return wstr;
}
...
{
    char str[MAX_PATH];
    if (!WideCharToMultiByte(CP_ACP, MB_PRECOMPOSED, wstr.c_str(), -1, str, MAX_PATH, nullptr, nullptr)) str[0] = L'\0';
    return str;
}
...
std::string Utility::GetFileExtension(const std::string& filePath)
{
    ...
    if (extOffset == std::wstring::npos) return "";
...
std::wstring Utility::GetFileExtension(const std::wstring& filePath)
{
    ...
    if (extOffset == std::wstring::npos) return L"";
